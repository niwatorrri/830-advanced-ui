...
public class TalkUI extends InteractiveWindowGroup {
    ...
    // Parameters to connect to GCP dialogflow
    private String sessionId;
    private String projectId;

    /**
     * Main entry point of the interface
     * 
     * @param args should pass in sessionId and projectId; alternatively run `export
     *             GOOGLE_APPLICATION_CREDENTIALS=/path/to/your-project-credentials.json`
     *             with your GCP credentials
     */
    ...
    public TalkUI(String sessionId, String projectId) {
        ...
        Group[] panels = makeGUI();
        Group controlPanel = panels[0];
        Group drawingPanel = panels[1];
        MicrophoneAnalyzer mic = new MicrophoneAnalyzer();
        TextToSpeech tts = new TextToSpeech();
        listenForVoiceInput(mic, controlPanel, drawingPanel, tts);
    }

    private void listenForVoiceInput(MicrophoneAnalyzer mic, Group controlPanel, Group drawingPanel, TextToSpeech tts) {
        while (mode == Mode.DRAW) {
            mic.open();
            ...
            if (isSpeaking) {
                try {
                    ...
                    if (audioLength > 1) { // to avoid abrupt noise
                        ...
                        makeResponse(mic, controlPanel, drawingPanel, tts);
                    } else {
                        System.out.println("Recording Dismissed");
                    }
                } catch (Exception e) {
                    e.printStackTrace();
    ...
    private void makeResponse(MicrophoneAnalyzer mic, Group controlPanel, Group drawingPanel, TextToSpeech tts) {
        ...
        controlPanel.addChild(new Text(detectedText));
        for (GraphicalObject o : controlPanel.getChildren()) {
    ...
    private Group[] makeGUI() {
        ...
        Widget<?> exportButton = new ButtonPanel(0, 0, false, ButtonPanel.MULTIPLE).addChild(new Button("Save Canvas"))
                .setCallback(o -> {
                    try {
        ...
        return new Group[] { voiceControlPlane, drawingPanel };
    }
}