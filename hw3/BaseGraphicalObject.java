...
public abstract class BaseGraphicalObject implements GraphicalObject {
	// this is just an example. Assume every object that inherits from this must have x,y and fields
	//  all the other properties will need the same kind of thing implemented
	int x = 0;
	int y = 0;
	// you would have similar fields for all other parameters that can be constrained
	...
	protected void noteValChange(Constraint<Integer> listOfUsers[], int newval) {
		...
		// might want to be smarter and only do this when the value is different than the old value
	}
	...
	public void setX (Constraint<Integer> newxc) { // two overloaded versions of setX - with a value or with a constraint
		/* first probably need to check if there was already a constraint there and clean up */
		xc = newxc;
